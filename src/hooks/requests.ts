...
export function useCreate<T extends object>(params: string) {
    ...
    await axios.post(`https://api0.loystar.co/api/v2/${params}`, payload, {
      headers: {
        client: user?.client,
...
export function useLoystarGetRequest<TData>(endpoint: string, payload: any) {
  ...
  export function useDelete() {
    ...
    async function deletes(params: string) {
      try {
        setLoading(true);
        await axiosRequest.delete(params);
      } catch (error: any) {
        processError(error);
      } finally {
        setLoading(false);
  ...
  export function useMutate<T extends object>(params: string) {
    ...
    async function mutating(payload: T) {
      try {
        setLoading(true);
    //    await axiosRequest.post({ url: params, payload: payload });

    await axios.put(`https://api0.loystar.co/api/v2/${params}`, payload, {
      headers: {
        client: user?.client,
        "access-token": user?.access_token,
        "uid": user?.uid
